...
namespace DDbook
{
    ...
    public class DB
    {
        ...
        public DB()
        {
            try
            {
                string s = "server= WIN-RQEIPQ15UPF\\ULTRAMAN;database=BookShop;uid=sa;pwd=sa";
        ...
        #region 无连接操作
        /// <summary>
        /// 无连接操作，加载查询的数据库，精确查询一行数据，并将查询到的数据存储在本地中
        /// </summary>
        ...
        /// <summary>
        /// 无连接操作，加载查询的数据库，精确查询一行数据，并将查询到的数据存储在本地中 value 为int型
        /// </summary>
        /// <param name="tableName">要查询的表名</param>
        /// <param name="listName">要查询的列名</param>
        /// <param name="value">要查询的值</param>
        ...
        /// <returns></returns>
        public bool Update(string listName, int value, int bid, int cid)
        {
            string sql = string.Format("update ShoppingCart set {0}={1} where BookID={2} and CustomerID={3}", listName, value, bid, cid);
            try
            {
                SqlCommand comm = new SqlCommand(sql, conn);
